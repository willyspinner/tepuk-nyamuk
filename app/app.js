...
app.set('dataURI','app/tempData/playersData.json');
app.use(express.static('app/public'));
app.set('ingame',false);// Whether we are in game or not.
//app.set('url',"http://localhost:3000");
app.set('url',app.get('host')+":"+app.get('port'));
...
fs.writeFile(app.get('dataURI'), '[]', 'utf8', function(err) {
          ...
          io.on('connection', function(socket) {
                    ...
                    socket.on('iThrew',function(card){
                              ...
                              //app.set('pile',app.get('pile').length); //fIX THIS WAS THE PROBLEM.
                                                  ...
                                                  if( card.card == app.get('counter')){
                                                            ...
                                                            /*socket.on('iSlap',function(data){
                                                                      ...
                                                                      if (!slappedUsers.includes(data['username'])){
                                                                                ...
                                                                                console.log(slappedUsers.length == app.get('nplayers')? "matchResult should be emitted now.":"" );
                                                                                if (slappedUsers.length == app.get('nPlayers'))
                                                                                          io.emit('matchResult',{'loser':slappedUsers.pop(),
                                                                                                                        'nCards':app.get('pile')+card['left'] });
                                                                      }
                                                            });*/
                                                  }
                              ...
                              if (!app.get('slappedUsers').includes(data['username'])){
                                        ...
                                        if (app.get('slappedUsers').length == app.get('nPlayers'))
                                        {
                                                  ...
                                                  fs.writeFile(app.get('dataURI'), JSON.stringify(playersData), 'utf8', function(err) {
                                                            if (err)
                                                                      console.log(err);
                                                            io.emit('matchResult',{'loser':loser/*app.get('slappedUsers').pop()*/,
                                                                                          'pile':app.get('pile')/*card['left']*/ });
                                                            app.set('pile',[]);//TODO CONCURRENCY ISSUE HERE. WHAT IF app.set IS CALLED BEFORE app.get()?
